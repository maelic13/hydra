# ruff: noqa: PT009
"""Board performance benchmarks — mirrors beast's test_board_performance.py.
...
class BoardPerformanceTest(unittest.TestCase):
    def test_engine_core_operations_have_reasonable_throughput(self) -> None:
        boards = tuple(Board.from_fen(fen) for fen in BENCHMARK_FENS)
        ...
        results = (
            ...
            self._benchmark(
                "perft(4) startpos",
                lambda: perft(Board.from_fen(STARTING_FEN), 4),
                3,
                warmups=1,
            ),
            self._benchmark(
    ...
    def _legal_moves(boards: tuple[Board, ...]) -> int:
        return sum(len(generate_legal_moves(b)) for b in boards)

    @staticmethod
    def _capture_gen(boards: tuple[Board, ...]) -> int:
        return sum(len(generate_captures(b)) for b in boards)
    ...
    def _game_simulation(boards: tuple[Board, ...]) -> int:
        ...
        for board in boards:
            for move in generate_legal_moves(board):
                board.make_move(move)
                # At ply 2, generate opponent's replies (the real workload)
